...
// 1. 拦截所有的“+”号，干掉两侧间距
#show math.plus: it => math.class("normal", it)

// 2. 拦截所有的“-”号（注意在数学里它是减号 minus）
#show math.minus: it => math.class("normal", it)

// 3. 拦截所有的“=”号
#show math.eq: it => math.class("normal", it)

// 4. 如果你用了其他符号（如 <, >, * ），也可以照葫芦画瓢加上：
#show math.lt: it => math.class("normal", it)
...
#columns(
  ...
  *WiMAX和LTE相同技术*:正交频分多址 OFDMA,子信道自适应调制和编码(AMC),混合自动重传请求 (H-ARQ),多输入多输出(MIMO),纯 IP 核心网
  ...
  #kp-mix-box(clr-green,[信道]) *物理信道*:用来传送信号或数据的物理通路,每个载频上支持的一个时隙(TS)就是一个物理信道;*逻辑信道*:物理信道上所传输的内容.根据物理信道所传输的信息种类的不同可定义不同的逻辑信道:#alert([业务信道和控制信道]);根据所需完成的功能,#success([控制信道])又分为:#alert([广播信道,公共控制信道,专用控制信道])
