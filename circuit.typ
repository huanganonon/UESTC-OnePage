...
#columns(3, gutter: 2mm)[
  ...
  #kp-mix-box(my-blue, [接收机性能指标]) 选择性（除/减少干扰信号和寄生频率信号影响）；灵敏度（保证特定通信距离和正确解调）；动态范围（确定正确解调的信号变化范围）。增益/隔离度/低失真
  ...
  #kp-mix-box(my-blue, [发射机性能指标]) 频谱纯度（减少邻道干扰）；功率（保证通信距离）；效率（减少系统功耗）

  #image("figures/直接变换.png")
  #kp-mix-box(my-red, [直接变换法]) #info([*特点*])：无镜像/中频干扰，结构简单，收发机同时开断切换；#info([*缺点*])：发射频率等于本振频率，发射强信号会影响本振源（载波泄漏） ，两路电特性不一致时达不到预期效果#info([*改进*])：可以使用两个较低本振合成

  // #image("figures/双本振直接变换.png")

  #kp-mix-box(my-red, [两步法/间接调制]) 在较低的频率上调制，再上变频到发射频率；克服了载波泄露；较低频率处调制容易，降低了滤波器要求，有助于复杂调制；对上变频滤波器要求高，存在变频组合干扰且结构复杂
  ...
  #kp-mix-box(my-red, [无耗线])$R = G = 0$,$alpha = 0$,$beta = omega sqrt(L C)$,$gamma = j beta$,$Gamma(z) = (V^- (z))/(V^+ (z)) = Gamma_0 e^(2 j beta z)$,$Gamma_("in") (z) = (Z_("in") (z) - Z_0)/(Z_("in") (z) + Z_0)$ #info([*相速*])：$lambda = v_P/f$,$v_P = 1/sqrt(L C)$,$beta = omega/v_P = (2pi)/lambda$ #alert([*驻波比*])：$"SWR" = (|V_("max")|)/(|V_("min")|) = (1+|Gamma|)/(1-|Gamma|)$ #alert([*输入阻抗*])：$Z_("in") (z) = Z_0 (1 + Gamma(z))/(1 - Gamma(z)) = Z_0 (Z_L - j Z_0 tan beta z)/(Z_0 - j Z_L tan beta z)$

  #kp-mix-box(my-red, [短路线]) $V(z) = -2 j V_0^+ sin beta z$,$I(z) = (2V_0^+)/Z_0 cos beta z$,$Z_("in") = -j Z_0 tan beta z$

  #kp-mix-box(my-red, [开路线]) $V(z) = 2 V_0^+ cos beta z$,$I(z) = (-2 j V_0^+)/Z_0 sin beta z$,$Z_("in") = j Z_0 cot beta z$
  ...
  #kp-mix-box(my-blue, [ABCD]) $mat(v_1; i_1) = mat(A, B; C, D) mat(v_2; i_2)$,$mat(v_1; i_1) = mat(A_1, B_1; C_1, D_1) mat(A_2, B_2; C_2, D_2) mat(v_3; i_3)$#common([互易])则$A D - B C = 1$;#common([*常用*])：#common([传输线])$mat(cos(beta l), j Z_0 sin(beta l); j Y_0 sin(beta l), cos(beta l))$;#common([T型]) $mat(1 + Z_A \/ Z_C, Z_A + Z_B + Z_A Z_B \/ Z_C; 1 \/ Z_C, 1 + Z_B\/ Z_C)$,#common([$pi$型])$mat(1 + Y_B \/ Y_C, 1 \/ Y_C; Y_A + Y_B + Y_A Y_B \/ Y_C, 1 + Y_A\/ Y_C)$
  ...
  #kp-mix-box(my-red, [Smith圆图]) #success([圆半径越小对应值越大]) #alert([*串联元件-阻抗圆图*])：串联L，沿#info([等电阻圆顺时针])；串联C，沿#info([等电阻圆逆时针])；#alert([*并联元件-导纳圆图*])：并联L，沿#info([等电导圆逆时针])；并联C，沿#info([等电导圆顺时针])；#success([*连接无耗线*])：等驻波比圆*顺时针（向电源方向）*或*逆时针（向负载方向）*。#success([*连接短截线*])：纯电抗（电纳）性质，并联使用等电导圆

  #image("figures/微带线结合电容.png", height: 7%)
  ...
  #kp-mix-box(my-red, [链路分析]) #common([*增益*])：由前向后的原则，注意$L = -G_p$(分贝)；#info([*噪声系数*])：由后向前的原则，#success([无源器件噪声系数等于其插入损耗]),使用级联计算公式；#alert([*三阶截断点*])：#info([由后向前])的原则，注意#common([无源器件通常不考虑])（太大）

  #kp-mix-box(my-green, [滤波器指标]) #common([*插入损耗*])：$"IL" = 10 lg P_("av")/P_L=-20 lg |S_21|$,对于无损网络$"IL" = 10 lg P_("in")/P_L = -10 lg (1 - |Gamma_"in" |^2)$($P_("in")$信号源输入功率)；#common([*波纹*])：衡量带内响应平坦度；#common([*带宽*])；#common([*矩形系数*])：带通滤波器通带与阻带间过渡带宽的陡峭程度$"SF" = "BW"_("3dB")/("BW"_("60dB"))$;#common([*品质因素*])：空载为在谐振频率下滤波器一个周期内平均储能与功率损耗的比$Q = omega_c W_("stored")/P_("loss")$;有载$Q_L$接入负载$1/Q_L = 1/Q + 1/Q_E$(外部品质因素),$"BW"_("3dB") = f_c/Q_L$

  #kp-mix-box(my-red, [低通滤波器原型]) #alert([*巴特沃斯*])：$"IL" = 10 lg(1 + a^2 Omega^(2N))$,#success([归一化频率])$Omega = omega/omega_c$,$omega_c$为截止频率,N为滤波器阶数,a取1时$Omega=1$是3dB损耗点；#alert([*切比雪夫*])：$I L = 10 lg([1 + a^2 T_"N"^2(Omega)])$,$T_"N" (Omega) = cos{N [cos^(-1)(Omega)]}|Omega| <= 1$,$T_"N" (Omega) = cosh{N [cosh^(-1)(Omega)]} |Omega| >= 1$,这里a是波纹幅度调节因子$a^2 = 10^("RP"_"IL" \/ 10) - 1$,$"RP"_"IL"$是波纹峰值(dB)；#common([*原型电路*])：$g_0$第一个元件是并联电容则表示电阻，串联电感则表示电导；$g_(N + 1)=1$负载看最后一个元件决定电阻/电导
  #image("figures/原型电路-横向.png")
  ...
  #kp-mix-box(my-red, [稳定条件]) $Delta = S_11 S_22 - S_12 S_21$,$Gamma_("in") = S_11 + (S_12 S_21 Gamma_L) / (1 - S_22 Gamma_L) = (S_11 - Gamma_L Delta)/(1-S_22 Gamma_L)$,$Gamma_("out") = S_22 + (S_12 S_21 Gamma_S) / (1 - S_11 Gamma_S) = (S_22 - Gamma_S Delta)/(1-S_11 Gamma_S)$;#success([*绝对稳定*])：任何无源负载和源阻抗,$|Gamma_("in")|<1$和$|Gamma_("out")|<1$;#success([*条件稳定*])：限定条件下稳定

  #kp-mix-box(my-blue, [稳定性判别圆]) #alert([*输出稳定圆*])：令$|Gamma_("in")| = 1$,有$C_("in") = ((S_22 - Delta S_11^*)^*) / (|S_22|^2 - |Delta|^2)$,$r_("in") = (|S_12 S_21|) / (|S_22|^2 - |Delta|^2)$；#success([*判定*])：令$Gamma_L = 0$(原点),$Gamma_("in") = |S_11|$,$|S_11|<1$则#success([原点在稳定区域内])；$|S_11|>1$则原点是非稳定区域
  #image("figures/输出稳定圆.png")
  #alert([*输入稳定圆*])：令$|Gamma_("out")| = 1$,有$C_("out") = ((S_11 - Delta S_22^*)^*) / (|S_11|^2 - |Delta|^2)$,$r_("out") = (|S_12 S_21|) / (|S_11|^2 - |Delta|^2)$；#success([*判定*])：令$Gamma_S = 0$(原点),$Gamma_("out") = |S_22|$
  #image("figures/输入稳定圆.png")

  #kp-mix-box(my-blue, [稳定因子]) $k = (1 - |S_11|^2 - |S_22|^2 + |Delta|^2) / (2 |S_12 S_21|) > 1$,$|Delta|<1$($|S_11|<1$,$|S_22|<1$才可能绝对稳定)

  #kp-mix-box(my-blue, [功率增益]) *资用功率*:输出端能够输出的最大功率，必须$Z_"in" = Z_S^*$,$P_A = (|V_S|^2)/(8R_S) = (|V_S|^2)/(8Z_0) (|1 - Gamma_S|^2)/(1-|Gamma_S|^2)$;*输入功率*：放大器或网络输入端实际接收到的功率$P_("in") = (|V_("in")|^2)/(2|Z_("in")|^2) Re(Z_"in")$;*输出功率*：信号源实际输出到负载上的功率，即负载消耗的功率$P_("out") = (|V_("out")|^2)/(2|Z_("L")|^2) Re(Z_"L")$,$P_L = (|V_S|^2) / (8 Z_0) (|1 - Gamma_S|^2 |S_21|^2 (1 - |Gamma_L|^2)) / (|1 - S_22 Gamma_L|^2 |1 - Gamma_S Gamma_("in")|^2) = (|V_S|^2) / (8 Z_0)(|1 - Gamma_S|^2 |S_21|^2 (1 - |Gamma_L|^2)) / (|1 - S_11 Gamma_S|^2 |1 - Gamma_L Gamma_("out")|^2)$;#success([*功率转换增益*])：负载吸收功率与信号源资用功率之比，$G_T = (P_L) / (P_A) = ((1 - |Gamma_S|^2) |S_21|^2 (1 - |Gamma_L|^2)) / (|1 - S_22 Gamma_L|^2 |1 - Gamma_S Gamma_("in")|^2) = ((1 - |Gamma_S|^2) |S_21|^2 (1 - |Gamma_L|^2)) / (|1 - S_11 Gamma_S|^2 |1 - Gamma_L Gamma_("out")|^2)$（与信号源内阻及负载有关）；#success([*资用功率增益*])：负载可能获得的最大功率与信号源的资用功率之比$G_A = G_T bar_(Gamma_L = Gamma_("out")^*) = (P_(L "max")) / (P_A) = ((1 - |Gamma_S|^2) |S_21|^2) / (|1 - S_11 Gamma_S|^2 (1 - |Gamma_("out")|^2))$（与信号源有关，与负载无关）；#success([*工作功率增益*])：放大器输出功率与输入功率之比$G_P = (P_L) / (P_("in")) = (P_L) / (P_A) (P_A) / (P_("in")) = G_T (P_A) / (P_("in")) = (|S_21|^2 (1 - |Gamma_L|^2)) / (|1 - S_22 Gamma_L|^2 (1 - |Gamma_("in")|^2))$(与信号源无关，与负载有关);#common([*匹配关系*])：输入匹配时$P_("in") = P_A => G_T = G_P$,输出匹配时$P_("L") = P_(L "max") => G_T = G_A$,输入输出都匹配时$G_T = G_A = G_P$

  #kp-mix-box(my-purple, [最大增益设计]) $Gamma_S^* = Gamma_("in")$,$Gamma_L^* = Gamma_("out")$,得$(S_22^* Delta - S_11) Gamma_S^2 + (1 + |S_11|^2 - |S_22|^2 - |Delta|^2) Gamma_S + (S_11^* - Delta^* S_22) = 0$,解这个方程$Gamma_S = (-B plus.minus sqrt(B^2 + 4 |A|^2))/(2A)$,同理$Gamma_L = (B_2 plus.minus sqrt(B_2^2 + 4|C_2|^2))/(2C_2)$($B_2 = 1 + |S_22|^2 - |S_11|^2 - |Delta|^2$,$C_2 = S_22 - Delta S_11^*$)
]
